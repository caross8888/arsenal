...
const ENTRIES = [
  ...
  ['빅토르 요케레스', ['빅토르 기외케레스', '빅토르 교케레스', '빅토르 죄케레스', /\bViktor Gy(ö|o)keres\b/g]],
  ['요케레스', ['기외케레스', '교케레스', '죄케레스', /(?<![A-Za-z])Gy(ö|o)keres\b/g]],
  ...
  ['맥스 다우먼', ['맥스 도우먼', '맥스 도우만', '맥스 다우만', '맥스 도먼', /\bMax Dowman\b/g]],
  // "도먼" 단독은 앞에 한글이 붙어 있으면 안 잡는다(단어 중간 — "포도먼저" 등).
  ['다우먼', ['도우먼', '도우만', '다우만', /(?<![가-힣])도먼/g, /\bDowman\b/g]],
  ...
  // 예컨대 Salmon(새먼)은 영어 성만 잡으면 "연어"까지 건드리게 되므로 영어는 풀네임만 넣는다.
  // 반면 한글 음차 '살몬'은 번역 결과에서 연어 뜻으로 쓰일 일이 없어 성 단독도 잡는다
  // (구글이 새먼을 살몬으로 옮김). 앞에 한글이 붙은 경우(단어 중간)는 건드리지 않는다.
  ['말리 새먼', ['말리 살몬', /\bMarli Salmon\b/g]],
  ['새먼', [/(?<![가-힣])살몬/g]],
