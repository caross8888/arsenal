...
suite('이름', applyGlossary, [
  ...
  ['빅토르 죄케레스의 결승골', '빅토르 요케레스의 결승골'],
  ['죄케레스가 교체 투입됐다', '요케레스가 교체 투입됐다'],
  ...
  ['맥스 도먼이 이번 시즌 출전 시간을', '맥스 다우먼이 이번 시즌 출전 시간을'],
  ['아르테타는 도먼에게 기회를 줬다', '아르테타는 다우먼에게 기회를 줬다'],
  ['맥스 도우만이 두 골을 넣었다', '맥스 다우먼이 두 골을 넣었다'],
  ['다우만의 활약', '다우먼의 활약'],
  ['포도먼저 먹고 가자', '포도먼저 먹고 가자'],                   // 단어 중간의 "도먼"은 안 잡음
...
suite('용어', applyGlossary, [
  // 스코틀랜드 히버니언 — 기사에선 별칭 Hibs로만 나온다
  ['Hibs 감독 후보', '히버니언 감독 후보'],
  ['히브스 감독 후보', '히버니언 감독 후보'],
  ['Hibernian FC 소식', '히버니언 소식'],
...
// ── 2-1. @핸들 보호 ──────────────────────────────────────────────────
// 핸들은 계정 주소라 표기 통일 대상이 아니다 — "@Arsenal"을 "@아스날"로 바꾸면
// 존재하지 않는 계정이 된다(SNS 카드에서 실제로 발생).
suite('핸들', applyGlossary, [
  ['@Arsenal 막으려면 특별한 게 필요했다', '@Arsenal 막으려면 특별한 게 필요했다'],
  ['@BHAFC가 무패 행진을 끝냈다', '@BHAFC가 무패 행진을 끝냈다'],
  ['아스널이 이겼다 @Arsenal', '아스날이 이겼다 @Arsenal'],   // 본문은 고치고 핸들은 그대로
]);
